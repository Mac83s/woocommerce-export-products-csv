...
namespace WoocommerceExportProductsCSVVendor\WPDesk\Tracker;

use WoocommerceExportProductsCSVVendor\WPDesk\PluginBuilder\Plugin\Hookable;
use WoocommerceExportProductsCSVVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\DirResolver;
class OptInPage implements \WoocommerceExportProductsCSVVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function __construct($plugin_file, $plugin_slug, $shop = null)
    {
        ...
        $this->shop = $shop ?? new \WoocommerceExportProductsCSVVendor\WPDesk\Tracker\Shop('');
    }
    ...
    public function output()
    {
        ...
        if (isset($_GET['shop_url'])) {
            $shop = new \WoocommerceExportProductsCSVVendor\WPDesk\Tracker\Shop(\sanitize_text_field(\wp_unslash($_GET['shop_url'])));
        } else {
            $shop = $this->shop;
        }
        ...
        $renderer = new \WoocommerceExportProductsCSVVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
    ...
    public function admin_init()
    {
        if (isset($_GET['wpdesk_tracker']) && $_GET['wpdesk_tracker'] === $this->plugin_slug) {
            if (isset($_GET['allow']) && isset($_GET['security']) && \wp_verify_nonce($_GET['security'], $this->plugin_slug)) {
                if ($_GET['allow'] === '1') {
                    $persistence = new \WoocommerceExportProductsCSVVendor\WPDesk_Tracker_Persistence_Consent();
