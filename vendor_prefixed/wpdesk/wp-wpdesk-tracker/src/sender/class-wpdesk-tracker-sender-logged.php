...
if (!\class_exists('WoocommerceExportProductsCSVVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        private function do_send(array $payload) : array
        {
            ...
            try {
                ...
                return $response;
            } catch (\WoocommerceExportProductsCSVVendor\WPDesk_Tracker_Sender_Exception_WpError $e) {
        ...
        {
            if (\class_exists('WoocommerceExportProductsCSVVendor\\WPDesk_Logger_Factory')) {
                \WoocommerceExportProductsCSVVendor\WPDesk_Logger_Factory::log_message('Sender payload: ' . \json_encode($payload), self::LOGGER_SOURCE, \WoocommerceExportProductsCSVVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \WoocommerceExportProductsCSVVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \WoocommerceExportProductsCSVVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\WoocommerceExportProductsCSVVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \WoocommerceExportProductsCSVVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
