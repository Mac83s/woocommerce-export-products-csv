 ...
 */
class WordpressFilterStorage implements \WoocommerceExportProductsCSVVendor\WPDesk\PluginBuilder\Storage\PluginStorage
{
    ...
    public function add_to_storage($class, $object)
    {
        \add_filter(self::STORAGE_FILTER_NAME, static function ($plugins) use($class, $object) {
            if (isset($plugins[$class])) {
                throw new \WoocommerceExportProductsCSVVendor\WPDesk\PluginBuilder\Storage\Exception\ClassAlreadyExists("Class {$class} already exists");
            }
    ...
    public function get_from_storage($class)
    {
        ...
        throw new \WoocommerceExportProductsCSVVendor\WPDesk\PluginBuilder\Storage\Exception\ClassNotExists("Class {$class} not exists in storage");
    }
}
