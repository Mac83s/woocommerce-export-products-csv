...
namespace WoocommerceExportProductsCSVVendor\WPDesk\View;

use WoocommerceExportProductsCSVVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\ChainResolver;
use WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\DirResolver;
use WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \WoocommerceExportProductsCSVVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
