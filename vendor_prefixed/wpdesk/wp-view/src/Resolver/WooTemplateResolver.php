 ...
 */
class WooTemplateResolver implements \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\Resolver
{
    /** @var string */
    private $base_path;
    ...
    public function resolve($name, \WoocommerceExportProductsCSVVendor\WPDesk\View\Renderer\Renderer $renderer = null) : string
    {
        $template = \wc_locate_template($name, '', $this->base_path);
        if ($template === '') {
            throw new \WoocommerceExportProductsCSVVendor\WPDesk\View\Resolver\Exception\CanNotResolve("Cannot resolve template {$name}");
        }
        return $template;
    }
}
